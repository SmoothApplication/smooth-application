...
exports.run = async function(ctx){
  ...
  try {
    ...
    await goToSessionByPill(page, 3); // trip — its own session again, 6 required fields of its own.
    ...
    // trip's own name/purpose/workStatus/traveldate/appdate filled but returndate invalid, that's 5 of
    // trip's own 6 fields, i.e. 83%, not the 100% it would wrongly show if the raw non-empty value counted.
    var progressText = await page.$eval('#sessionProgressPct', function(el){ return el.textContent; });
    assert.ok(/83% filled/.test(progressText), 'Session should read 83% filled (return date not counted) while dates are invalid, got: ' + progressText);
    ...
    assert.ok(/100% filled/.test(progressTextAfterFix), 'Session should read 100% filled once the return date is valid (all 6 of trip\'s fields), got: ' + progressTextAfterFix);
