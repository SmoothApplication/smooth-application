...
exports.run = async function(ctx){
  ...
  try {
    ...
    // auto-advances to Step 2 (the cash-flow table). Wait for #stmtAnalyzeMsg's own "Detected N
    // transaction(s)" text instead of the box itself — it's set as the LAST step of the whole analysis
    // sequence (after the auto-advance and every box render), so by the time it appears it's safe to
    // navigate to Step 5 without racing anything further.
