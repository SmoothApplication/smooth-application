...
// web/app/checklist/uk/business-income/page.tsx for UK's own dedicated route, same split as every
// other per-country sub-route in this app (financial/statement/passport/situation).
export function generateStaticParams() {
  return Object.keys(COUNTRY_CHECKLISTS).map((code) => ({ country: code.toLowerCase() }));
}

export default function CountryBusinessIncomePage({ params }: { params: { country: string } }) {
  const data = COUNTRY_CHECKLISTS[params.country.toUpperCase()];
  if (!data) notFound();

  return <BusinessIncomeLedger countryCode={data.code} />;
}
