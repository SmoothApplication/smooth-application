...
// country's /checklist/<country>/financial route, not just the UK's. computeFinancials() itself
// (lib/checklist/financial.ts) is pure math with no country assumptions — this component is just
// the form + display around it, now parameterized by `countryCode` instead of hardcoding "uk".
//
// Privacy: unchanged — everything typed here stays in this browser's localStorage (key
// sa_<countryCode>_financial) and is never sent anywhere.
//
...
export type FinancialCalculatorProps = {
  countryCode: string;
};
