...
throughout the app's user-facing text (`index.html`), but never updated the test files' own
assertion strings/regexes to match, so several tests kept expecting the old em-dash text against
output that had already changed. Fixed six test files to match what the app actually renders now:
`business-income-record.test.js`, `financial-summary-table.test.js`,
`trip-length-row-removed.test.js`, `matched-income-inflows-itemized.test.js`,
`matched-inflow-collapse-on-blur.test.js`, and `new-session-order.test.js`. Two other flagged
lines (`dob-digit-misread-fixture.test.js`, `application-tracker.test.js`) were checked and left
as-is - neither compares against em-dash-swept app output, so neither was actually broken.

Separately, a real run turned up one more casualty of the Reasons-tab work itself (unrelated to
em dashes): `europe-funds-question.test.js` still checked its "this doesn't affect your
checklist/block anything" reassurance inline on the single-entry follow-up question, but that text
sits inside a `details.tip-details` toggle - one of the 21 relocated into the Reasons tab. Updated
the test to check the Reasons tab instead, matching how `finance-tip-details.test.js` already
handles the same situation. A full search confirmed no other test still checks a relocated
`tip-details` toggle's content inline.

Separately again: `passport-camera-scan.test.js` is failing consistently (two identical runs) at
its post-capture wait, unrelated to any of the above - the camera-capture wiring wasn't touched
today. Root cause not yet found. Added console/pageerror diagnostics to the test plus fixed a
...
the "mockup 2b" redesign made that `<select>` an intentionally-hidden element driving the logic
behind a tappable list instead. Fixed both to wait for the visible `.gate-country-option`, matching
how `passConsentGate()` in `helpers.js` already does it.

## Added a "Reasons" tab and moved explanations out of the main flow

User request: reduce how much explanatory text sits directly in the app, and put the reasoning
behind specific decisions somewhere separate instead of next to every field. Added a floating
"📖 Reasons" tab, visible on every screen this app has (the quiz, the consent gate, every session -
it's a fixed sibling of `<body>`, not nested inside any of them), that opens a modal listing every
relocated explanation, grouped by which session it came from.

What moved: all 21 of the existing "Why?" `<details>` toggles across every session (passport,
travel history, financial calculator, bank statements, and the landing screen's own FAQ), plus five
specific always-visible paragraphs that were pure framing/rationale rather than something needed to
fill a field correctly - the travel-history country-guide intro, the rent-estimate framing note, the
Opportunities directory intro, the application tracker's own framing note, and the founder/credit
paragraph in the footer.

What deliberately stayed in place, on purpose: every checklist item's own description of what the
document is and why it's needed (this is core content, regenerated fresh on every render - moving it
would have meant applicants no longer see what a checklist item actually requires); validation rules
that state an actual constraint rather than just rationale (e.g. the closing-balance figure that must
match the total of uploaded statements); dynamic status and prompt text in the bank-statement
analyzer; and every privacy/consent/legal notice (the "this is guidance, not immigration advice"
disclaimer, the "nothing leaves your device" notes, the contact-consent text). Those all matter in
the moment they're shown, not after the fact, and several are regenerated by JS after the page loads
- a one-time sweep would have either missed them or broken them.

## Simplified the bank statement upload step to one account first

Design direction from the same mockup deck (round 2, screen 2d): Step 1 of Income & bank statement
analysis now leads with a single statement picker instead of three at once - a "+ Add another
account" link reveals a second, then a third, only if actually needed. A small "🔒 Runs on your
phone" note sits right next to the Analyze button (moved out of the paragraph below it, where it
was easy to skim past). Once analysis finishes, the done-bar now reads "Ready · N month(s) detected"
instead of a bare "analyzed" - the one number most worth confirming at a glance.

## Added a guided live-camera scan for the passport bio page

Design direction from the same mockup deck (round 2, screen 2c): a "📷 Scan with camera" option
next to the existing "📎 Scan passport" file upload, opening a live camera view with a framing
guide over where the MRZ strip should sit. A capture that comes out too dark is rejected with a
"try better light" message rather than being sent through 20-30 seconds of OCR that was never going
to read it; a good capture flows into the exact same on-device reading pipeline the file upload
already uses. After a scan, the passport expiry field is scrolled to and highlighted automatically,
since OCR is never certain enough on an expiry date to skip a manual look.

Camera access is entirely optional - if the browser has no camera support, permission is declined,
or the applicant would just rather not, the plain file upload (and the manual "type it in" fields
below it, which were already there regardless of scan) work exactly as before. Also fixed the
site's Permissions-Policy header, which had camera locked to `()` (disabled everywhere) from before
any camera feature existed - needed updating to `camera=(self)` for this to work in production at
all.

## Redesigned the country picker on the consent gate as a tappable list

Design direction from the "SA MOCK UP" deck (round 2, screen 2b): replaced the plain country
`<select>` dropdown with a tappable list of the 7 supported countries, each showing its visa name
and a "ready" or "coming soon" badge. Picking a ready country shows a confirmed-summary row
("UK - Standard Visitor visa", with a Change link) instead of leaving the raw list on screen.

Under the hood the original `<select id="gateCountrySelect">` is unchanged and still drives every
existing bit of app logic (disclaimer text, the quiz prefill, the Continue button) - it's just
visually hidden now, with the new list acting as its view. Test suite updated: Playwright's
`selectOption()` needs a visible element, so every test that picks a country now clicks the
matching list option instead.

## Added a short FAQ to the landing screen

Design direction from the same mockup deck (screen 2a): three collapsible questions under the
Start/Skip buttons - why trust a free tool with this, does this predict approval, and which
countries are supported - using the existing "Why?" disclosure style already used elsewhere in the
app, rather than a new UI pattern.
