...
exports.run = async function(ctx){
  ...
  try {
    ...
    // Name/purpose/dates are all filled, but "Work status" is still on its unselected placeholder —
    // should NOT read 100% (the bug reported: "100% complete" while "Work status" itself sat unanswered).
    ...
    assert.ok(afterFilled && /100% filled/.test(afterFilled), 'Should read 100% again once business name is filled (and everything else in "Your trip details" is complete), got: ' + afterFilled);
