...
// Phase 2 of task #244: the real UK checklist body. Two views in one page — a short qualifying
// form (mirrors what index.html calls "Your responsibilities", trimmed to just the fields
// CHECKLIST_UK's appliesIf functions actually key off) and the categorized document list itself.
//
// Privacy: exactly like index.html, none of this — answers or checked state — is sent anywhere.
// It lives in this browser's localStorage only (keys namespaced sa_uk_*, separate from the
// original site's storage since this is a different origin). Only email/country/percent go to
// Supabase, and only once/if the applicant drops an email elsewhere in the flow.
const ANSWERS_KEY = 'sa_uk_answers';
const CHECKED_KEY = 'sa_uk_checked';
...
export default function UKChecklistPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (a) setView('checklist');
    } catch {
      // localStorage unavailable — start fresh, same as a first-time visitor.
    }
    setLoaded(true);
  }, []);
      ...
      <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-5 p-8">
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-black/10 bg-white p-4 text-sm">
          ...
          <label className="flex items-center gap-2">
            ...
            I&apos;m staying with a host in the UK (not a hotel)
          </label>
          {answers.hasHost && (
            <label className="ml-6 flex items-center gap-2 text-[#4c6270]">
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-5 p-6 pb-16">
      <div className="sticky top-0 z-10 -mx-6 border-b border-black/10 bg-[#f7fafb]/95 px-6 py-3 backdrop-blur">
        <div className="flex items-center justify-between text-sm">
          <span className="font-semibold text-[#12232e]">🇬🇧 UK Standard Visitor checklist</span>
      ...
      {CAT_ORDER_UK.map((cat) => {
        const items = CHECKLIST_UK.filter((it) => it.cat === cat && itemApplies(it, answers));
...
function ChecklistRow({
  ...
  item: (typeof CHECKLIST_UK)[number];
  checked: boolean;
  onToggle: () => void;
}) {
  return (
