...
// web/app/checklist/uk/next-steps/page.tsx for UK's own dedicated route, same split as every other
// per-country sub-route in this app.
export function generateStaticParams() {
  return Object.keys(COUNTRY_CHECKLISTS).map((code) => ({ country: code.toLowerCase() }));
}

export default function CountryNextStepsPage({ params }: { params: { country: string } }) {
  const data = COUNTRY_CHECKLISTS[params.country.toUpperCase()];
  if (!data) notFound();

  return <NextStepsReport countryCode={data.code} />;
}
